...
| px-pixelart | private | Scrubbed | Yes | Scrubbed | Yes | Added 2026-09-25; re-mirrored 2026-09-26 (inserting rows and columns, black blank squares in dark mode). README: removed Getting started (commands). User Guide copied as-is (no commands, paths or env vars). Testing Guide: removed local setup, server commands, the storage-filling console snippet, local URLs, rebuild steps and the automated-checks section. Tutorial from `src/components/TutorialModal.jsx`. |
